CMD:acmds(playerid)
{
	...
	strcat(line3, "\n"BLUE_E"Exiled:RP "WHITE_E"- Anti-Cheat is actived.\n\
	"PINK_E"NOTE: All admin commands log is saved in database! | Abuse Commands? Kick And Demote Premanent!.");
	ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""ORANGE_E"Exiled:RP: "YELLOW_E"Staff Commands", line3, "OK","");
	return true;
}
...
CMD:respawnsapd(playerid, params[])
{
	...
	for(new i = 1; i < MAX_VEHICLES; i ++)
	{
		if(PoliceVehicle{i})
	    {
			if(IsVehicleEmpty(PoliceVehicle{i}))
			{
				DestroyVehicle(i);
	        	PoliceVehicle{i} = false;
			}
		}
	}

	SendStaffMessage(COLOR_RED, "%s has respawned POLISI vehicles.", pData[playerid][pAdminname]);
	return 1;
}
...
CMD:respawnsags(playerid, params[])
{
	...
	for(new i = 1; i < MAX_VEHICLES; i ++)
	{
		if(GovVehicle{i})
	    {
			if(IsVehicleEmpty(GovVehicle{i}))
			{
				DestroyVehicle(i);
	        	GovVehicle{i} = false;
			}
	...
	SendStaffMessage(COLOR_RED, "%s has respawned PEMERINTAH vehicles.", pData[playerid][pAdminname]);
	return 1;
}
...
CMD:respawnsamd(playerid, params[])
{
	...
	for(new i = 1; i < MAX_VEHICLES; i ++)
	{
		if(MedicVehicle{i})
	    {
			if(IsVehicleEmpty(MedicVehicle{i}))
			{
				DestroyVehicle(i);
	        	MedicVehicle{i} = false;
			}
		}
	}
	SendStaffMessage(COLOR_RED, "%s has respawned EMS vehicles.", pData[playerid][pAdminname]);
	return 1;
}

CMD:respawnnews(playerid, params[])
{
	if(pData[playerid][pAdmin] < 1)
		if(pData[playerid][pHelper] < 3)
			return PermissionError(playerid);
	...
	for(new i = 1; i < MAX_VEHICLES; i ++)
	{
		if(NewsVehicle{i})
	    {
			if(IsVehicleEmpty(NewsVehicle{i}))
			{
				DestroyVehicle(i);
	        	NewsVehicle{i} = false;
			}
	...
	SendStaffMessage(COLOR_RED, "%s has respawned NEWS vehicles.", pData[playerid][pAdminname]);
	return 1;
}
