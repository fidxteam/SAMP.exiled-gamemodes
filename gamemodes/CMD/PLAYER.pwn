...
CMD:help(playerid, params[])
{
	...
	ShowPlayerDialog(playerid, DIALOG_HELP, DIALOG_STYLE_LIST, "Exiled Help Menu", info, "Select", "Close");
	return 1;
}
...
CMD:credits(playerid)
{
	...
	strcat(line3, ""LB_E"Coder: "YELLOW_E"Adit overall , Parel for Update\n");
	strcat(line3, ""LB_E"Helped by: "YELLOW_E"Raul\n");
	strcat(line3, ""LB_E"Managed by: "YELLOW_E"Fery (Silence)\n");
	strcat(line3, ""LB_E"Support Mapper: "YELLOW_E"Fery and his Friends\n");
	strcat(line3, ""LB_E"Support Website: "YELLOW_E"Parel\n");
	format(line2, sizeof(line2), ""LB_E"Server Support: "YELLOW_E"%s & All SA-MP Team\n\n\
	"GREEN_E"Terima kasih telah bergabung dengan kami! Copyright © 2020 | Raul.", pData[playerid][pName]);
	format(line1, sizeof(line1), "%s%s", line3, line2);
   	ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""ORANGE_E"Exiled:RP: "WHITE_E"Server Credits", line1, "OK", "");
	return 1;
}
...
CMD:vip(playerid)
{
	...
	"YELLOW_E"Untuk informasi selengkapnya hubungi Raul/Fery(Server Owner & Founder)!");
	ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""ORANGE_E"Exiled:RP "PINK_E"VIP SYSTEM", longstr2, "Close", "");
	return 1;
}
...
{
    new line3[3500];
    strcat(line3, ""RED_E"...:::... "DOOM_"Donate List Exiled Roleplay "RED_E"...:::...\n");
    ...
    strcat(line3, ""DOOM_"1. 7 Days >> "RED_E"Rp 15.000/Boost Discord\n");
    strcat(line3, ""DOOM_"2. 14 Days >> "RED_E"Rp 25.000/Boost Discord\n");
    ...
    strcat(line3, ""DOOM_"1. VIP Regular(1 Month) >> "RED_E"Rp 15.000\n");
    strcat(line3, ""DOOM_"2. VIP Premium(1 Month) >> "RED_E"Rp 25.000\n");
    strcat(line3, ""DOOM_"3. VIP Platinum(1 Month) >> "RED_E"Rp 50.000\n\n");

	strcat(line3, ""RED_E"..:::.. "DOOM_"SERVER FEATURE "RED_E"..:::..\n\n");
    strcat(line3, ""DOOM_"1. Mapping(per object)
	strcat(line3, ""DOOM_"2. Private Door
	strcat(line3, ""DOOM_"3. Private Gate
	strcat(line3, ""DOOM_"4. Bisnis
	strcat(line3, ""DOOM_"5. House
	strcat(line3, ""DOOM_"6. Custom House Interior 
	
	strcat(line3, ""RED_E"..::::.. "DOOM_"SERVER VEHICLE "RED_E"..:::::..\n\n");
    strcat(line3, ""DOOM_"1. VEHICLE IN DEALER >> "RED_E"Rp 10.000\n");
	strcat(line3, ""DOOM_"2. VEHICLE NON DEALER >> "RED_E"Rp 20.000\n");
	strcat(line3, ""DOOM_"3. BOAT / HELI >> "RED_E"Rp 25.000\n\n");

    strcat(line3, ""RED_E"..::.. "WHITE_E"CONTACT INFO "RED_E"..::..\n");
    strcat(line3, ""WHITE_E"1. NAMA : "RED_E"Gores(Raul)\n");
    strcat(line3, ""WHITE_E"-  WA: "RED_E"082139345719\n\n");
	
    strcat(line3, ""WHITE_E"2. NAMA : "RED_E"Aero(Fery)\n");
    strcat(line3, ""WHITE_E"-  WA : "RED_E"085749793427\n\n");

    strcat(line3, ""RED_E"..::.. "WHITE_E"NOTE "RED_E"..::..\n");
    strcat(line3, ""WHITE_E"Note: "RED_E"Pembayaran Via Rekening Bank BRI, PayPal, Gopay, Dana, Saweria!\n\n");

	ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""ORANGE_E"Exiled: "WHITE_E"DONATE LIST", line3, "Okay", "");
	return 1;
}
...
CMD:gshop(playerid, params[])
{
	...
	Instant Change Name\t9999999 Gold\n");
...
CMD:time(playerid)
{
	...
	format(line2, sizeof(line2), ""WHITE_E"Paycheck Time: "YELLOW_E"%d remaining\n"WHITE_E"Delay Job: "RED_E"%d Detik\n"WHITE_E"Delay Side Job: "RED_E"%d Detik\n"WHITE_E"Plant Time(Farmer): "RED_E"%d Detik\n"WHITE_E"Arrest Time: "RED_E"%d Detik\n"WHITE_E"Jail Time: "RED_E"%d Detik\n", paycheck, pData[playerid][pJobTime], pData[playerid][pSideJobTime], pData[playerid][pPlantTime], pData[playerid][pArrestTime], pData[playerid][pJailTime]);
   	ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""ORANGE_E"Exiled RP: "WHITE_E"Time", line2, "Oke", "");
	return 1;
}
	...
	foreach(new i : PVehicles)
	{
		if(vehid == pvData[i][cVeh])
		{
			if(pvData[i][cOwner] == pData[playerid][pID])
			{
				...
				if(IsValidDynamic3DTextLabel(pvData[i][cPlate3D]))
				{
					DestroyDynamic3DTextLabel(pvData[i][cPlate3D]);
					pvData[i][cPlate3D] = Text3D:INVALID_3DTEXT_ID;
					
					pvData[i][cPlate3D] = CreateDynamic3DTextLabel(pvData[i][cPlate], COLOR_GREY2, 0.0, -3.0, 0.0, 10.0, .attachedvehicle = pvData[i][cVeh]);
				}
			}
...
CMD:togtw(playerid, params[])
{
	...
	if(pData[playerid][pUsePhone] < 1)
		return Error(playerid, "Anda belum menyalakan Ponsel");
...
CMD:tw(playerid, params[])
{
	if(IsPlayerInRangeOfPoint(playerid, 50, 2184.32, -1023.32, 1018.68))
		return Error(playerid, "Sistem Ini tidak dapat digunakan di OOC Zone");
	...
	if(pData[playerid][pTogPhone] < 1)
		return Error(playerid, "Anda belum membuka twitter [/togtw]");

    if(!strcmp(pData[playerid][pTwittername], "None"))
		return Error(playerid, "Kamu harus setting Nama oleh Admin mu!");

	if(isnull(params))
	{
	    Usage(playerid, "/tw <text>");
	    return true;
	}
...
{
	if(pData[playerid][pPhone] == 0) return Error(playerid, "Anda tidak memiliki Ponsel!");
	if(pData[playerid][pUsePhone] == 0) return Error(playerid, "Handphone anda sedang dimatikan");
...
{
	if(pData[playerid][pUsePhone] == 0) return Error(playerid, "Tidak dapat iklan, Ponsel anda sedang Offline");
...
CMD:buy(playerid, params[])
{
	...
	foreach(new gid : Garages)
	{
		...
		{
			if(GetPlayerMoney(playerid) < GarageData[gid][garagePrice]) return Error(playerid, "Not enough money, you can't afford this private garage.");
			if(strcmp(GarageData[gid][garageOwner], "-")) return Error(playerid, "Someone already owns this private garage.");
			GivePlayerMoneyEx(playerid, -GarageData[gid][garagePrice]);
			Server_AddMoney(-GarageData[gid][garagePrice]);

			format(GarageData[gid][garageOwner], MAX_PLAYER_NAME, pData[playerid][pName]);
			GarageData[gid][garageOwnerID] = pData[playerid][pID];
...
CMD:starterpack(playerid)
{
	...
	if(pData[playerid][pStarterpack] == 1)
		return Error(playerid, "Anda sudah mengambil starterpack");
	
	Info(playerid, "Kamu mendapatkan starterpack sebesar %s", FormatMoney(jumlah));
	GivePlayerMoneyEx(playerid, jumlah);
