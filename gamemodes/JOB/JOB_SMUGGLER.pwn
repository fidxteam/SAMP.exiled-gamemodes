...
{
	if(pData[playerid][pJob] != 12 && pData[playerid][pJob2] != 12) return Error(playerid, "You're not Smuggler");
...
function Readd_Packet(Float:x, Float:y, Float:z)
{
    ...
    packetLabel = CreateDynamic3DTextLabel("{7fffd4}SMUGGLER PACKET\n{ffffff}Press '{ffff00}N{ffffff}' to pickup", -1, x, y, z+0.3, 10.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1, -1, -1, -1);
    return 1;
}
...
task SmugglerRand[3000000]()
{
    if(!IsPacketTaken())
    {
        ...
        foreach(new p : Player) if(pData[p][pJob] == 12 || pData[p][pJob2] == 12)
        {
