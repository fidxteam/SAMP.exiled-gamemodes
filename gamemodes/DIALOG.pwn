...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    ...
    if(dialogid == DIALOG_LOGIN) 
    {
        ...
        {
            new cagus[250];
            format(cagus, sizeof cagus, ""WHITE_E"Selamat datang kembali "YELLOW_E"%s"WHITE_E", silahkan masukkan password Anda dibawah:", GetName(playerid));
            ShowPlayerDialog(playerid, DIALOG_LOGIN, DIALOG_STYLE_PASSWORD, "LOGIN", cagus, "Masuk", "Batal");
            return 1;
        }
        ...
        if(strcmp(hash, UcpData[playerid][uPassword])) 
        {
            ...
            {
                new cagus[350];
                format(cagus, sizeof cagus, ""WHITE_E"Selamat datang kembali "YELLOW_E"%s"WHITE_E", silahkan masukkan password Anda dibawah:\n"RED_E"Password salah! Sisa kesempatan: %d / 3", GetName(playerid), UcpData[playerid][uLoginAttempts]);
                ShowPlayerDialog(playerid, DIALOG_LOGIN, DIALOG_STYLE_PASSWORD, "LOGIN", cagus, "Masuk", "Batal");
            }
            return 1;
        }
	...
	if(dialogid == DIALOG_VERIFYCODE)
	{
		if(response)
		{
			...
			if(isnull(inputtext))
			{
				format(str, sizeof(str), ""WHITE_E"Selamat datang"YELLOW_E" %s\n{ffffff}Silahkan masukkan PIN yang sudah di kirimkan oleh BOT lewat DM", GetName(playerid));
				return ShowPlayerDialog(playerid, DIALOG_VERIFYCODE, DIALOG_STYLE_INPUT, "Verifikasi Akun", str, "Masuk", "Batal");
			}
			else if(!IsNumeric(inputtext))
			{
				format(str, sizeof(str), ""WHITE_E"Selamat datang"YELLOW_E" %s\n{ffffff}Silahkan masukkan PIN yang sudah di kirimkan oleh BOT lewat DM\n\n"RED_E"ERROR: Pin hanya berisi sebuah angka!", GetName(playerid));
				return ShowPlayerDialog(playerid, DIALOG_VERIFYCODE, DIALOG_STYLE_INPUT, "Verifikasi Akun", str, "Masuk", "Batal");	
			}
			else if(strval(inputtext) == UcpData[playerid][uPIN])
			{
				UcpData[playerid][uPassword] = UcpData[playerid][uSalt] = EOS;
				new cagus[250];
				format(cagus, sizeof cagus, ""WHITE_E"Akun dengan nama "YELLOW_E"%s "WHITE_E"sudah terverifikasi. Silahkan masukkan password baru anda:", GetName(playerid));
				ShowPlayerDialog(playerid, DIALOG_REGISTER, DIALOG_STYLE_PASSWORD, "Registrasi Password", cagus, "Registrasi", "Tutup");
				return 1;
			}

			format(str, sizeof(str), ""WHITE_E"Selamat datang"YELLOW_E" %s\n{ffffff}Silahkan masukkan PIN yang sudah di kirimkan oleh BOT lewat DM\n\n"RED_E"ERROR: Pin Salah", GetName(playerid));
    ...
    if(dialogid == DIALOG_REGISTER)
    {
        ...
        else 
        {
            ...
            {
                new cagus[250];
				format(cagus, sizeof cagus, ""WHITE_E"Akun dengan nama "YELLOW_E"%s "WHITE_E"sudah terverifikasi. Silahkan masukkan password baru anda:\n\n"GREY_E"ERROR: Password terlalu singkat, masukkan dengan minimal 8 karakter.", GetName(playerid));
                ShowPlayerDialog(playerid, DIALOG_REGISTER, DIALOG_STYLE_PASSWORD, "Membuat Password", cagus, "Buat", "Keluar");
                return 1;
            }
            for (new i; i < 64; i++)
    ...
    if(dialogid == DIALOG_PASSWORD)
    {
        if(response)
        {
            ...
            new query[500];
            mysql_format(g_SQL, query,sizeof(query), "UPDATE ucp SET password = '%s', salt = '%s', ip = '%s' WHERE username = '%s'", UcpData[playerid][uPassword], UcpData[playerid][uSalt], UcpData[playerid][uIP], UcpData[playerid][uUsername]);
	...
	if(dialogid == DIALOG_MY_BISNIS)
	{
		...
		ShowPlayerDialog(playerid, BISNIS_INFO, DIALOG_STYLE_LIST, "{FF0000}Exiled:RP {0000FF}Bisnis", "Show Information\nTrack Bisnis", "Select", "Cancel");
		return 1;
	}
			...
			else if(bData[bizid][bType] == 3)
			{
				switch(listitem)
				{
					...
					case 1:
					{
						...
						ShowPlayerDialog(playerid, DIALOG_TOYBUY, DIALOG_STYLE_LIST, ""RED_E"Exiled RP: "WHITE_E"Player Toys", string, "Select", "Cancel");
					}
					case 2:
					{
						GivePlayerMoneyEx(playerid, -price);
	...
	if(dialogid == DIALOG_MY_HOUSES)
	{
		...
		ShowPlayerDialog(playerid, HOUSE_INFO, DIALOG_STYLE_LIST, "{FF0000}Exiled:RP {0000FF}Houses", "Show Information\nTrack House", "Select", "Cancel");
		return 1;
	}
	...
	if(dialogid == DIALOG_FIND_DEALER)
	{
		if(response)
		{
			...
			Info(playerid, "The Dealer checkpoint targeted! (%s)", GetLocation(DealerData[id][dealerPosX], DealerData[id][dealerPosY], DealerData[id][dealerPosZ]));
		}
	}
		...
		if(response) 
		{
			...
			foreach(new i : PVehicles)
			{
				if(carid == pvData[i][cVeh])
				{
					...
					format(pvData[i][cPlate], 32, "EX-%d", rand);
			...
			switch(listitem)
			{
				case 0: //slot 1
				{
					...
					else
					{
						...
						ShowPlayerDialog(playerid, DIALOG_TOYEDIT, DIALOG_STYLE_LIST, ""RED_E"Exiled:RP "WHITE_E"Player Toys", string, "Select", "Cancel");
					}
				}
				case 1: //slot 2
				{
					...
					else
					{
						...
						pToys[playerid][pData[playerid][toySelected]][toy_rx], pToys[playerid][pData[playerid][toySelected]][toy_ry], pToys[playerid][pData[playerid][toySelected]][toy_rz]);
						ShowPlayerDialog(playerid, DIALOG_TOYEDIT, DIALOG_STYLE_LIST, ""RED_E"Exiled:RP "WHITE_E"Player Toys", string, "Select", "Cancel");
						//ShowPlayerDialog(playerid, DIALOG_TOYEDIT, DIALOG_STYLE_LIST, ""RED_E"Exiled:RP "WHITE_E"Player Toys", ""dot"Edit Toy Position\n"dot"Change Bone\n"dot""GREY_E"Remove Toy\n"dot"Share Toy Pos", "Select", "Cancel");
					}
				}
				case 2: //slot 3
				{
		...
		if(response)
		{
			switch(listitem)
			{
				...
				case 1: // change bone
				{
					...
					ShowPlayerDialog(playerid, DIALOG_TOYPOSISI, DIALOG_STYLE_LIST, ""RED_E"Exiled RP: "WHITE_E"Player Toys", finstring, "Select", "Cancel");
				}
				case 2: // remove toy
				{
	...
	if(dialogid == DIALOG_HELP)
    {
		...
		switch(listitem)
		{
			...
			case 5:
			{
				...
				ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""RED_E"Exiled AUTO RP", str, "Close", "");
			}
			case 6:
			{
				new str[3500];
		...
		if(response)
		{
			switch (listitem) 
			{
				...
				case 3:
				{
					...
					ShowPlayerDialog(playerid, DIALOG_TOYVIP, DIALOG_STYLE_LIST, ""RED_E"Exiled:RP "WHITE_E"VIP Toys", string, "Select", "Cancel");
				}
			}
		}
	}
	...
	if(dialogid == DIALOG_BANKDEPOSIT)
	{
		...
		else
		{
			...
			ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""ORANGE_E"Exiled RP: "LB_E"Bank", lstr, "Close", "");
		}
	}
		...
		if(response)
		{
			switch (listitem) 
			{
				case 0:
				{
					...
					ShowPlayerDialog(playerid, DIALOG_DOWNLOAD, DIALOG_STYLE_TABLIST_HEADERS, "Exiled App Store",string,"Download","Batal");
				}
				case 1:
				{
					new mstr[128];
