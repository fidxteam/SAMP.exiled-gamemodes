...
function OnUCPLoaded(playerid, race_check) 
{
    ...
    if (rows)
	{
        ...
        cache_get_value_name_int(0, "pin", UcpData[playerid][uPIN]);

		if(!strcmp(UcpData[playerid][uPassword], "None"))
		{
			new str[400];
			...
			ShowPlayerDialog(playerid, DIALOG_VERIFYCODE, DIALOG_STYLE_INPUT, "Verifikasi Akun", str, "Masuk", "Batal");
		}
		else
		{
			new cagus[250];
        ...
        /*new cagus[250];
        format(cagus, sizeof cagus, ""WHITE_E"Selamat datang kembali "YELLOW_E"%s"WHITE_E", silahkan masukkan password Anda dibawah:", GetName(playerid));
        ShowPlayerDialog(playerid, DIALOG_LOGIN, DIALOG_STYLE_PASSWORD, "Masuk", cagus, "Masuk", "Batal");*/
    }
    else
    {
        new cagus[250];
        format(cagus, sizeof cagus, ""WHITE_E"Akun dengan nama "YELLOW_E"%s "WHITE_E"tidak terdaftar. Silahkan mandaftarkan UCP anda di offical discord kami:", GetName(playerid));
        ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, "UCP Tidak Terdaftar", cagus, "Tutup", "");
		KickEx(playerid);
    }
...
{
	new str[32];
	format(str,sizeof(str),"%d Players", PlayerCount);
