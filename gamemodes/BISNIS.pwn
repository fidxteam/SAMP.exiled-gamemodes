...
Bisnis_Refresh(id)
{
    if(id != -1)
    {
        ...
        if(strcmp(bData[id][bOwner], "-"))
		{
			...
			bData[id][bPickup] = CreateDynamicPickup(19198, 23, bData[id][bExtposX], bData[id][bExtposY], bData[id][bExtposZ]+0.2, 0, 0, _, 8.0);
        }
        else
        {
...
CMD:mybis(playerid)
{
	...
	ShowPlayerDialog(playerid, DIALOG_MY_BISNIS, DIALOG_STYLE_LIST, "{FF0000}Basic:RP {0000FF}Bisnis", CMDSString, "Select", "Cancel");
	return 1;
}
